...
저장소에 `dark.zip`, `violet.zip` 샘플 테마가 포함되어 있습니다.  
`/admin/settings/theme` 에서 바로 업로드해 테마 시스템을 테스트할 수 있습니다.

| 항목 | default | dark | violet |
|------|---------|------|--------|
| 네비게이션 | 흰 배경 + 하단 선 | `#0f172a` 다크 네이비 | 흰 배경 + 바이올렛 브랜드 |
| 포인트 컬러 | Bootstrap 파랑 `#0d6efd` | 인디고 `#6366f1` | 바이올렛 `#7c3aed` |
| 버튼 | 파란 계열, 직각 | 인디고 계열, 직각 | 바이올렛, 풀 라운드 |
| 카드 hover | 없음 | 없음 | 위로 이동 + 보라 그림자 |
| 게시판 헤더 | 회색 | 회색 | 연보라 (`#ede9fe`) |
| 히어로 | — | 다크 그라디언트 | 딥 퍼플 그라디언트 + 빛 원형 |
| 푸터 | 다크 | 다크 + 인디고 | 다크 퍼플 `#1e1b2e` |
| SNS 아이콘 | 정적 | hover 인디고 + 위 이동 | hover 라벤더 + 위 이동 |
| 썸네일 | — | 없음 (직접 추가 가능) | ✓ `thumbnail.png` 포함 |
