...
| 회원 인증 | 회원가입(이메일 인증) · 로그인 · 로그아웃 · 소셜 로그인(구글·네이버·카카오) |
| 회원정보 | 휴대폰번호 · 성별 · 생일 · 주소(카카오 주소 API, 선택) |
| 이메일 인증 | 가입 후 인증 메일 발송 → 24시간 이내 클릭 시 활성화, 재발송(1분 쿨다운) |
...
| 테이블 | 설명 |
|--------|------|
| `users` | 회원 (admin / member 역할, phone · gender · birthday · email_verify_token) |
...
| `/auth/login` | 로그인 |
| `/auth/register` | 회원가입 |
| `/auth/verify-pending` | 이메일 인증 대기 안내 |
| `/auth/verify/{token}` | 이메일 인증 처리 |
...
| 항목 | 변경 내용 |
|------|----------|
| **이메일 인증 가입** | 가입 후 즉시 로그인 → 이메일 인증 완료 후 활성화로 변경. 인증 토큰 24시간 만료, 재발송 1분 쿨다운, SMTP 미설정 시 로그 기록 |
| **회원 정보 확장** | `users` 테이블에 `phone`(필수) · `gender` · `birthday` · `email_verify_token` · `email_verify_token_at` 컬럼 추가 |
| **회원가입 폼** | 휴대폰번호(필수), 성별, 생일 입력 필드 추가. 주소 선택 입력 시 카카오 주소 API로 배송지 자동 저장 |
| **프로필 수정** | `auth/profile` 기본 정보 탭에 휴대폰번호 · 성별 · 생일 수정 기능 추가 |
| **인증 대기 화면** | `auth/verify-pending` — 인증 메일 발송 안내 + 재발송 버튼 신규 추가 |
| **로그인 개선** | 미인증 계정 로그인 시 "인증 필요" 메시지 + 재발송 버튼 표시. 비밀번호 정탐 방지를 위해 미인증 분기를 비밀번호 검증 전에 처리 |
| **CSS 버그 수정** | `.product-card`에 `overflow: hidden` 추가 → 상품 카드 이미지 모서리 깨짐 수정. 포인트 내역 카드 테이블 모서리 깨짐 수정 |
